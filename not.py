...
def register_user(message, username):
...
def start(message):
    ...
    if id:
        ...
        bot.register_next_step_handler(message, user_first_name, id)
        
        def user_first_name(message, id):
        ...
        markup = types.InlineKeyboardMarkup()
        retry_button = types.InlineKeyboardButton("Попробовать снова", callback_data="retry_registration")
        markup.add(retry_button)
        bot.send_message(message.chat.id, "У вас не установлен юзернейм в Telegram. Пожалуйста, установите его в настройках профиля и нажмите кнопку ниже для повторной попытки.", reply_markup=markup)
    
    @bot.callback_query_handler(func=lambda call: True)
    def handle_callback(call):
        username = call.from_user.username

        if call.data == "retry_registration":
            if username:
                # Если юзернейм теперь установлен, регистрируем пользователя
                register_user(message.chat.id, username)
                bot.send_message(message.chat.id, f"Вы успешно зарегистрированы с юзернеймом @{username}!")
            else:
                # Если юзернейма всё ещё нет, отправляем сообщение снова
                markup = types.InlineKeyboardMarkup()
                retry_button = types.InlineKeyboardButton("Попробовать снова", callback_data="retry_registration")
                markup.add(retry_button)
                bot.send_message(message.chat.id, "Юзернейм всё ещё не установлен. Пожалуйста, установите его и нажмите кнопку ниже.", reply_markup=markup)
